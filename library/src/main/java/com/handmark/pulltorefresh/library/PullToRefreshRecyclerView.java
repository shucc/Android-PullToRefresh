...
public class PullToRefreshRecyclerView extends PullToRefreshBase<RecyclerView> {
    ...
    protected RecyclerView createRefreshableView(Context context, AttributeSet attrs) {
        ...
        recyclerView.setId(R.id.default_pull_to_recycler_view);
        return recyclerView;
    }
    ...
    protected boolean isReadyForPullEnd() {
        ...
        if (mRefreshableView.getAdapter() != null) {
            if (lastVisiblePosition >= mRefreshableView.getAdapter().getItemCount() - 1) {
                return mRefreshableView.getChildCount() != 0 && mRefreshableView.getChildAt(mRefreshableView.getChildCount() - 1).getBottom() <= mRefreshableView.getBottom();
            }
        }
        return false;
    }
